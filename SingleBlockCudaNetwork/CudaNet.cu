...
void CudaNetwork::forward_prop(float* x_data, float* result_data, int activation_size, int num_elements) {
	...
	for (size_t i = 0; i < m_dimensions.size() - 1; i++) {
		...
		grid = (ceil(m_dimensions[i + 1] / 8.0f), ceil(num_elements / 8.0f), 1);


		i == 0 ?
			dot_prod_t_b << < grid, block >> > (weights, input, output, m_dimensions[i + 1], m_dimensions[i], num_elements, m_dimensions[i]) :
			dot_prod << < grid, block >> > (weights, input, output, m_dimensions[i + 1], m_dimensions[i], m_dimensions[i], num_elements);

		// add bias
		add_bias << <grid, block >> > (output, bias, m_dimensions[i + 1], num_elements);

		// activation
		leaky_relu << <grid, block >> > (output, &output[activation_size], m_dimensions[i + 1], num_elements);
